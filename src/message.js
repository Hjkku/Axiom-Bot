...
async function GroupUpdate(axiom, m, store) {
	...
	if (global.db?.groups?.[m.chat] && store?.groupMetadata?.[m.chat]) {
		...
		if (axiom.public && global.db?.groups?.[m.chat]?.setinfo && messages[type]) {
			await axiom.sendMessage(m.chat, { text: `${admin} ${messages[type]}`, mentions: [m.sender, ...((normalizedTarget?.id || normalizedTarget)?.includes('@') ? [`${normalizedTarget.id || normalizedTarget}`] : [])]}, { ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
		if (type === 20) {
			clearTimeout(groupMetadataTimers[m.chat])
			groupMetadataTimers[m.chat] = setTimeout(async () => {
				store.groupMetadata[m.chat] = await axiom.groupMetadata(m.chat).catch(e => ({ ...store.groupMetadata[m.chat] }));
			}, 5000);
			...
			if (m.fromMe && ((jidNormalizedUser(axiom.user.id) == (normalizedTarget.id || normalizedTarget)) || (jidNormalizedUser(axiom.user.lid) == (normalizedTarget.id || normalizedTarget)))) {
...
async function GroupParticipantsUpdate(axiom, { id, participants, author, action }, store) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				...
				let profile;
				try {
					profile = await axiom.profilePictureUrl(n, 'image');
				} catch {
				...
				if (action === 'add') {
					...
					if (!participant) {
						...
						groupMetadataTimers[id] = setTimeout(async () => {
							store.groupMetadata[id] = await axiom.groupMetadata(id).catch(e => ({ ...store.groupMetadata[id] }));
						}, 5000);
					}
				} else if (action === 'remove') {
					if (db.groups[id].leave) messageText = db.groups[id]?.text?.setleave || `@\nLeaving From ${metadata.subject}`;
					if ((jidNormalizedUser(axiom.user.lid) == jidNormalizedUser(n)) || (jidNormalizedUser(axiom.user.id) == jidNormalizedUser(n))) {
				...
				if (messageText && axiom.public) {
					await axiom.sendMessage(id, {
...
async function LoadDataBase(axiom, m) {
	try {
		const botNumber = await axiom.decodeJid(axiom.user.id);
...
async function MessagesUpsert(axiom, message, store) {
	try {
		let botNumber = await axiom.decodeJid(axiom.user.id);
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await axiom.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(axiom, msg, store)
		require('../axiom')(axiom, m, msg, store);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await axiom.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await axiom.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await axiom.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(axiom, store) {
	axiom.serializeM = (m) => MessagesUpsert(axiom, m, store)
	...
	axiom.findJidByLid = (lid, store, resolve = false) => {
	...
	axiom.getName = (jid, withoutContact  = false) => {
		const id = axiom.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = axiom.groupMetadata(id))) || {};
	...
	axiom.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await axiom.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await axiom.getName(i + '@s.whatsapp.net')}\nFN:${await axiom.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await axiom.getName(i + '@s.whatsapp.net')}\nFN:${await axiom.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/naze_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		axiom.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	axiom.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
	...
	axiom.extractGroupMetadata = (result) => {
	...
	axiom.groupMetadata = async (jid) => {
		...
		return axiom.extractGroupMetadata(result);
	};
	
	axiom.groupFetchAllParticipating = async () => {
		const result = await axiom.query({ tag: 'iq', attrs: { to: '@g.us', xmlns: 'w:g2', type: 'get' }, content: [{ tag: 'participating', attrs: {}, content: [{ tag: 'participants', attrs: {}}, { tag: 'description', attrs: {}}]}]});
		const data = {};
		const groupsChild = getBinaryNodeChild(result, 'groups');
		if (groupsChild) {
			const groups = getBinaryNodeChildren(groupsChild, 'group');
			for (const groupNode of groups) {
				const meta = axiom.extractGroupMetadata({
					tag: 'result',
					attrs: {},
					content: [groupNode]
				});
		...
		axiom.ev.emit('groups.update', Object.values(data));
		return data;
	}
	
	axiom.relayMessageV2 = async (jid, message, options) => {
		const msg = generateWAMessageFromContent(jid, message, {
			upload: axiom.waUploadToServer,
			messageId: generateMessageID(),
			...options
		});
		const hasil = await axiom.relayMessage(jid, msg.message, {
	...
	axiom.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return axiom.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	
	axiom.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return axiom.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return axiom.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return axiom.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return axiom.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return axiom.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return axiom.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	axiom.sendGroupInviteV4 = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await axiom.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	axiom.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await axiom.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	axiom.sendText = async (jid, text, quoted, options = {}) => axiom.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	
	axiom.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return axiom.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	axiom.downloadMediaMessage = async (message) => {
	...
	axiom.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await axiom.downloadMediaMessage(message);
	...
	axiom.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: axiom.user.id, quoted: m.quoted && m.quoted.fakeObj(), ephemeralExpiration: m.expiration || m?.metadata?.ephemeralDuration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
		apb.key.id = [...Array(32)].map(() => '0123456789ABCDEF'[Math.floor(Math.random() * 16)]).join('');
		apb.key.fromMe = areJidsSameUser(m.sender, axiom.user.id);
		if (m.isGroup) apb.participant = m.sender;
		axiom.ev.emit('messages.upsert', {
			...m,
	...
	axiom.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await axiom.getFile(path, true);
		...
		if (isWebpSticker) {
			pathFile = await writeExif(data, {
				...
				author: options.author || db?.set?.[botNumber]?.author || 'Rangga',
		...
		let anu = await axiom.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	axiom.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		await axiom.relayMessage(jid, medias.message, { messageId: medias.key.id });
		for (const media of album) {
			const msg = await generateWAMessage(jid, { ...others, ...media }, { upload: axiom.waUploadToServer });
			...
			await axiom.relayMessage(jid, msg.message, { messageId: msg.key.id });
		}
		return medias;
	}
	
	axiom.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await axiom.relayMessage(msg.key.remoteJid, msg.message, {
			messageId: msg.key.id,
			additionalNodes: [{
				tag: 'biz',
				attrs: {},
	...
	axiom.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	axiom.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: axiom.waUploadToServer });
			const anu = await axiom.query({
				tag: 'message',
			...
			const _query = await axiom.query({
				tag: 'iq',
				attrs: {
					to: 's.whatsapp.net',
					type: 'get',
	...
	axiom.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: axiom.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await axiom.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (axiom.user && axiom.user.id) {
		const botNumber = axiom.decodeJid(axiom.user.id);
		if (global.db?.set[botNumber]) {
			axiom.public = global.db.set[botNumber].public
		} else axiom.public = true
	} else axiom.public = true
...
async function Serialize(axiom, msg, store) {
	const botLid = axiom.decodeJid(axiom.user.lid);
	const botNumber = axiom.decodeJid(axiom.user.id);
	const m = { ...msg };
	if (!m) return m
	if (m.key) {
		...
		if (!m.isGroup && m.chat.endsWith('@lid')) m.chat = axiom.findJidByLid(m.chat, store) || m.chat;
		m.sender = axiom.decodeJid(m.fromMe && axiom.user.id || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await axiom.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = await axiom.groupMetadata(m.chat).catch(e => ({ ...store.groupMetadata[m.chat] })));
			if (!metadata) {
				metadata = await axiom.groupMetadata(m.chat).catch(e => ({ ...store.groupMetadata[m.chat] }));
			...
			if (metadata.addressingMode === 'lid') {
				...
				store.contacts[m.sender] = { ...store.contacts[m.sender], id: m.sender, lid: m.fromMe && axiom.user.lid || participant?.lid || m.sender, name: m.pushName };
			}
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted = {
				...
				sender: axiom.decodeJid(m.msg.contextInfo.participant),
				fromMe: axiom.decodeJid(m.msg.contextInfo.participant) === axiom.decodeJid(axiom.user.id),
			...
			m.quoted.fromMe = m.quoted.sender === axiom.decodeJid(axiom.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await global.loadMessage(m.chat, m.quoted.id, axiom)
				if (q) {
					return await Serialize(axiom, q, store)
				} else {
					return null
				}
			}
			m.quoted.key = {
				...
				fromMe: areJidsSameUser(axiom.decodeJid(m.msg?.contextInfo?.participant), axiom.decodeJid(axiom?.user?.id)),
				id: m.msg?.contextInfo?.stanzaId
			}
			...
			m.quoted.download = () => axiom.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				axiom.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => axiom.downloadMediaMessage(m)
	
	m.copy = () => Serialize(axiom, JSON.parse(JSON.stringify(m)), store)
	
	m.react = (u) => axiom.sendMessage(m.chat, { react: { text: u, key: m.key }})
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return axiom.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return axiom.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return axiom.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
					}
				} else {
					return axiom.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
				}
