...
global.fetchApi = async (path='/', data={}, options={}) => {
  return new Promise(async (resolve, reject) => {
    try {
      const base = options.name ? (options.name in global.APIs ? global.APIs[options.name] : options.name) : global.APIs.axiom
...
/*
	* Create By Axiom
	* Follow https://github.com/axiomdev
	* Whatsapp : https://whatsapp.com/channel/0029VaWOkNm7DAWtkvkJBK43
*/

async function startAxiomBot() {
	try {
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: 'Halo Saya Axiom Bot'
		}
	}
	
	const axiom = WAConnection({
		version,
		logger: level,
		getMessage,
	...
	if (pairingCode && !phoneNumber && !axiom.authState.creds.registered) {
	...
	axiom.ev.on('connection.update', async (update) => {
		...
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.connectionLost) {
				...
				axiom.end(`Unknown DisconnectReason : ${reason}|${connection}`)
			}
		}
		if (connection == 'open') {
			console.log('Connected to : ' + JSON.stringify(axiom.user, null, 2));
			let botNumber = await axiom.decodeJid(axiom.user.id);
			if (global.db?.set[botNumber] && !global.db?.set[botNumber]?.join) {
				if (my.ch.length > 0 && my.ch.includes('@newsletter')) {
					if (my.ch) await axiom.newsletterMsg(my.ch, { type: 'follow' }).catch(e => {})
	...
	axiom.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			let trueJid;
			if (!trueJid) continue;
			if (contact.id.endsWith('@lid')) {
				trueJid = axiom.findJidByLid(contact.id, store);
			} else {
	...
	axiom.ev.on('call', async (call) => {
		let botNumber = await axiom.decodeJid(axiom.user.id);
		if (global.db?.set[botNumber]?.anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await axiom.sendMessage(id.from, { text: `Saat Ini, Kami Tidak Dapat Menerima Panggilan ${id.isVideo ? 'Video' : 'Suara'}.\nJika @${id.from.split('@')[0]} Memerlukan Bantuan, Silakan Hubungi Owner :)`, mentions: [id.from]});
					await axiom.sendContact(id.from, global.owner, msg);
					await axiom.rejectCall(id.id, id.from)
				}
			}
		}
	});
	
	axiom.ev.on('messages.upsert', async (message) => {
		await MessagesUpsert(axiom, message, global.store);
	});
	
	axiom.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(axiom, update, global.store);
	});
	
	axiom.ev.on('groups.update', (update) => {
	...
	axiom.ev.on('presence.update', ({ id, presences: update }) => {
	...
	if (!global._dbPresence) {
		global._dbPresence = setInterval(async () => {
			if (axiom?.user?.id) await axiom.sendPresenceUpdate('available', axiom.decodeJid(axiom.user.id)).catch(e => {})
		}, 10 * 60 * 1000);
	}
